//Metodo de Eliminacao de Gauss usando a estrategia de pivo parcial
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
double Pivo(int N,double**matriz,double b[N],int L);
//Programa principal
int main(){
    int N;
    int i,j;
    printf("Por favor, tenha certeza de inserir um sistema SPD.\n");
...
//Estrategia do pivo parcial
double Pivo(int N,double**matriz,double b[N],int L){
    int i,j,k;
    double aux,aux1;
    for(i=L;i<N-1;i++){
        for(j=i;j<N;j++){
            if(matriz[i][L]<matriz[j][L]){
                for(k=L;k<N;k++){
                    aux = matriz[i][k];
